...
interface LeadSpendChartProps {
  totalRevenue?: number
  history?: { date: string; revenue: number }[]
}

export default function LeadSpendChart({ totalRevenue = 0, history = [] }: LeadSpendChartProps) {
  ...
  useEffect(() => {
    ...
    const base: MonthData[] = saved ? JSON.parse(saved) : BASE_DATA
    const updated = [...base]
    // Inject live revenue into current month
    updated[updated.length - 1] = { ...updated[updated.length - 1], spend: currentSpend, revenue: totalRevenue }
    setData(updated)
  }, [totalRevenue])
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setData(prev => {
        ...
        if (last.spend === currentSpend && last.revenue === totalRevenue) return prev
        const updated = [...prev]
        updated[updated.length - 1] = { ...updated[updated.length - 1], spend: currentSpend, revenue: totalRevenue }
        return updated
      })
  ...
  const totalRevenueSum = data.reduce((s, d) => s + d.revenue, 0)
  ...
  return (
    <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-light)', padding: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
        {[
          { label: 'Revenue',  value: totalRevenueSum > 0 ? fmtK(totalRevenueSum) : '—', color: '#22c55e' },
          { label: 'Spend',    value: totalSpend > 0   ? fmtK(totalSpend)   : '—', color: '#f59e0b' },
          { label: 'Profit',   value: totalRevenueSum > 0 || totalSpend > 0 ? fmtK(totalRevenueSum - totalSpend) : '—', color: 'var(--accent)' },
        ].map(s => (
