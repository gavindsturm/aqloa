...
import {
  Bell, Settings, Calculator, Users as UsersIcon, Calendar as CalendarIcon,
...
import InsuranceToolkit from './components/InsuranceToolkit'
...
export default function Dashboard() {
  ...
  const handleUpdateLead = (lead: Lead) => {
    ...
    setShowToolkit(false); setSelectedLead(null)
  }
  ...
  const openToolkit = (lead: Lead) => { setSelectedLead(lead); setShowToolkit(true) }
  ...
  const handleCSVUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = event => {
      ...
      rows.forEach((row, index) => {
        const [name, phone, email, source, value, age] = row.split(',').map(s => s.trim())
        if (name && email) newLeads.push({ id: Date.now() + index, name, phone: phone || '', email, source: source || 'CSV Import', status: 'new', value: value || '$500K', assignedTo: userEmail, time: 'Just now', notes: '', age: parseInt(age) || 35, smoker: false, medications: [], healthConditions: [] })
      })
      setLeads([...leads, ...newLeads]); alert(`Imported ${newLeads.length} leads`); e.target.value = ''
    }
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-base)' }}>
            ...
            {activeTab === 'dashboard' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }} className="card-stagger">
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <button onClick={() => handleTabChange('leads')} style={btnPrimary}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                          ...
                          <div onClick={e => { e.stopPropagation(); openToolkit(lead) }}>
                            <button style={{ padding: '5px 8px', background: 'var(--bg-overlay)', border: '1px solid var(--border-light)', borderRadius: 0, cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4, fontSize: 11 }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    <button onClick={() => { setSelectedLead(viewingLeadDetail); setShowToolkit(true) }} style={btnPrimary}>
                      <Calculator size={14} /> {viewingLeadDetail.selectedCarrier ? 'Recalculate' : 'Calculate Premium'}
                    </button>
                  </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16 }}>
                    ...
                    {viewingLeadDetail.selectedCarrier && viewingLeadDetail.monthlyPremium && (
                      <div style={{ ...card, overflow: 'hidden' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
                          ...
                          <button onClick={() => { setSelectedLead(viewingLeadDetail); setShowToolkit(true) }} style={btnSecondary}><Calculator size={12} /> Recalculate</button>
                        </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <div style={{ ...card, padding: 18 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <button onClick={() => { setSelectedLead(viewingLeadDetail); setShowToolkit(true) }} style={{ ...btnPrimary, width: '100%', justifyContent: 'center' }}><Calculator size={13} /> {viewingLeadDetail.selectedCarrier ? 'Recalculate' : 'Calculate Premium'}</button>
      ...
      {showToolkit && selectedLead && (
        <InsuranceToolkit lead={selectedLead} onClose={() => { setShowToolkit(false); setSelectedLead(null) }} onUpdate={handleUpdateLead} />
      )}
