...
export default function TeamManagement({ currentUser, profilePic }: Props) {
  const { members, removeMember, updateMember, pendingInvites, addPendingInvite, removePendingInvite, getDownline, getUpline, leaderboard } = useTeam()

  const [activeTab, setActiveTab] = useState<Tab>('overview')
  const [showInviteModal, setShowInviteModal] = useState(false)
  const [expandedMember, setExpandedMember] = useState<number | null>(null)
  const [editingMember, setEditingMember] = useState<number | null>(null)
  const [editFields, setEditFields] = useState<{ chargebacks: number; debt: number; overrides: string; agentNotes: string } | null>(null)
  ...
  // Only show actual invited downline — no fallback to other members
  const allDownline = downline
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* Stats strip */}
      <div className="team-stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
      ...
      {/* ── OVERVIEW ── */}
      {activeTab === 'overview' && (
        <div className="team-layout" style={{ display: 'grid', gridTemplateColumns: '0.85fr 1fr', gap: 16, alignItems: 'start' }}>
          <div className="team-leaderboard-col"><TeamPerformance currentUser={currentUser} profilePic={profilePic} /></div>
          <div className="team-chat-col"><TeamChat currentUser={currentUser} /></div>
        </div>
      )}
      ...
      {/* ── DOWNLINE ── */}
      {activeTab === 'downline' && (
        <div className="team-members-table" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>Agents under your supervision. Click any row to expand.</p>
          {/* Table header */}
          <div className="team-members-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 90px', gap: 0, padding: '8px 16px', background: 'var(--bg-elevated)', border: '1px solid var(--border-light)' }}>
          ...
          {allDownline.map(member => {
            ...
            return (
              <div key={member.id} style={{ ...card, overflow: 'hidden' }}>
                <div className="team-members-grid" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 90px', gap: 0, padding: '12px 16px', cursor: 'pointer', alignItems: 'center' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        <RankBadge xp={member.xp ?? 0} size="xs" />
                      </div>
                ...
                {expanded && (() => {
                  const isEdit = editingMember === member.id
                  const fmtD = (n: number) => n >= 1000 ? `$${(n/1000).toFixed(1)}K` : `$${n}`
                  return (
                    <div style={{ padding: '14px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
                      {/* Stats row */}
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12, marginBottom: 14 }}>
                        {[
                          { label: 'Closed', value: String(member.closed) },
                          { label: 'Revenue', value: `$${(member.revenue/1000).toFixed(1)}K` },
                          { label: 'Joined', value: member.joinedDate },
                          { label: 'Chargebacks', value: (member.chargebacks ?? 0) > 0 ? String(member.chargebacks) : '—', color: (member.chargebacks ?? 0) > 0 ? '#ef4444' : undefined },
                          { label: 'Debt', value: (member.debt ?? 0) > 0 ? fmtD(member.debt!) : '—', color: (member.debt ?? 0) > 0 ? '#f59e0b' : undefined },
                        ].map(item => (
                          <div key={item.label}>
                            ...
                            <p style={{ fontSize: 14, fontWeight: 700, color: (item as any).color ?? 'var(--text-primary)' }}>{item.value}</p>
                          </div>
                        ))}
                      </div>
                      {member.overrides && !isEdit && (
                        <div style={{ marginBottom: 12, padding: '8px 12px', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
                          <p style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 3 }}>Override</p>
                          <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{member.overrides}</p>
                        </div>
                      )}
                      {/* Edit form */}
                      {isEdit && editFields && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                          <div>
                            <p style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Chargebacks</p>
                            <input type="number" min={0} value={editFields.chargebacks}
                              onChange={e => setEditFields(f => f ? { ...f, chargebacks: parseInt(e.target.value)||0 } : f)}
                              style={inputStyle} />
                          </div>
                          <div>
                            <p style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Debt ($)</p>
                            <input type="number" min={0} value={editFields.debt}
                              onChange={e => setEditFields(f => f ? { ...f, debt: parseFloat(e.target.value)||0 } : f)}
                              style={inputStyle} placeholder="0" />
                          </div>
                          <div style={{ gridColumn: '1/-1' }}>
                            <p style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Override Arrangement</p>
                            <textarea rows={2} value={editFields.overrides}
                              onChange={e => setEditFields(f => f ? { ...f, overrides: e.target.value } : f)}
                              placeholder="e.g. 5% override on FEX production from downline..."
                              style={{ ...inputStyle, resize: 'none' }} />
                          </div>
                          <div style={{ gridColumn: '1/-1' }}>
                            <p style={{ fontSize: 9, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>Notes (private)</p>
                            <textarea rows={2} value={editFields.agentNotes}
                              onChange={e => setEditFields(f => f ? { ...f, agentNotes: e.target.value } : f)}
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        {!isEdit ? (
                          <button onClick={() => { setEditingMember(member.id); setEditFields({ chargebacks: member.chargebacks ?? 0, debt: member.debt ?? 0, overrides: member.overrides ?? '', agentNotes: member.agentNotes ?? '' }) }}
                            style={{ ...btnSecondary, fontSize: 11 }}>Edit</button>
                        ) : (
                          <>
                            <button onClick={async () => {
                              if (editFields) await updateMember(member.id, editFields)
                              setEditingMember(null); setEditFields(null)
                            }} style={{ ...btnPrimary, fontSize: 11 }}>Save</button>
                            <button onClick={() => { setEditingMember(null); setEditFields(null) }} style={{ ...btnSecondary, fontSize: 11 }}>Cancel</button>
                          </>
                        )}
                      </div>
                    </div>
