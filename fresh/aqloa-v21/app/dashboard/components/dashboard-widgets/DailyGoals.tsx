...
export interface GoalMetric { key: string; label: string; current: number; target: number; icon: any; color: string }
interface Props {
  goals: GoalMetric[]
  onChange?: (key: string, current: number) => void
}

const COLOR: Record<string, string> = {
  blue: 'var(--accent)', purple: 'var(--accent)', green: 'var(--accent)',
  orange: 'var(--accent)', amber: 'var(--text-muted)', red: '#f87171',
}

export default function DailyGoals({ goals: initialGoals, onChange }: Props) {
  const [goals, setGoals] = useState(initialGoals)
  const [editingIndex, setEditingIndex] = useState<number | null>(null)
  const [editValues, setEditValues] = useState({ current: 0, target: 0 })

  useEffect(() => { setGoals(initialGoals) }, [JSON.stringify(initialGoals)])

  const startEdit = (idx: number) => {
    setEditingIndex(idx)
    setEditValues({ current: goals[idx].current, target: goals[idx].target })
  }
  ...
  const saveEdit = (idx: number) => {
    ...
    if (onChange) onChange(updated[idx].key, editValues.current)
  }
